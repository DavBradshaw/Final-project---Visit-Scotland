...
demographics %>% 
  ...
  theme(legend.position = "none")+
  labs(x = "Demographic category, stack by type",
...
scotland_rdt %>% 
  ...
  select(location, average_nights_per_year_millions, average_visits_per_year_thousands, average_spend) %>% 
  arrange(desc(average_spend))
...
activities %>% 
  filter(tourism_activity != "All") %>% 
  ggplot(aes(x = year, y = visits, colour = tourism_activity))+
  geom_line(alpha = 0.5)+
  geom_smooth(method = lm, se = FALSE)

activities %>% 
  filter(tourism_activity != "All") %>% 
  ggplot(aes(x = year, y = gbp_spent, colour = tourism_activity))+
  geom_line(alpha = 0.5)+
  geom_smooth(method = lm, se = FALSE)

locations %>% 
  filter(type_of_location_visited != "All areas") %>% 
  ggplot(aes(x= year, y = visits, colour = type_of_location_visited))+
  geom_line(alpha = 0.5)+
  geom_smooth(method = lm, se = FALSE)

locations %>% 
  filter(type_of_location_visited != "All areas") %>% 
  ggplot(aes(x= year, y = gbp_spent, colour = type_of_location_visited))+
  geom_line(alpha = 0.5)+
  geom_smooth(method = lm, se = FALSE)
